...
const getAllJobs = async (req, res) => {
  ...
  if(search){
   ...
   const limitt = Number(req.query.limit) || 50 

    const skipp = (page - 1) * limitt

    result = result.skip(skipp).limit(limitt)

   const jobs = await result

    const totalJobs = await Job.countDocuments(queryObject)
    const numOfPages = Math.ceil(totalJobs / limitt)
...
const updateJob = async (req, res) => {
 ...
 if(!position || !company){
  ...
  checkPermissions(req.user.userId.userID, job.createdBy)
...
const showStats = async (req, res) => {
 let stats = await Job.aggregate([
  {$match : {createdBy: new mongoose.Types.ObjectId(req.user.userId.userID)}},
